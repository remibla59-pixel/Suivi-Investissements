...
const InvestmentTrackerApp = () => {
  ...
  // NOUVEAU: État pour le tri des comptes
  ...
  // NOUVEAU: Fonction de tri pour les comptes
  const getSortedAccounts = (accounts, broker) => {
    ...
    return [...sortableAccounts].sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case 'value':
            aValue = a.currentValueEur;
            bValue = b.currentValueEur;
            break;
        case 'invested':
            aValue = a.investedTotalEur;
            bValue = b.investedTotalEur;
            break;
        case 'performance':
  ...
  const handleSaveMovement = (data) => {
      ...
      saveUserData(updated, undefined, undefined);
      const ub = updated.find(b => b.id === selectedBroker.id); setSelectedBroker(ub); setSelectedAccount(ub.accounts.find(a => a.id === selectedAccount.id)); showToast('Mouvement ajouté'); closeModal();
  };
  ...
  const handleExport = () => {
    const blob = new Blob([JSON.stringify({ brokers, patrimonyGoal, targetAllocation, version: "1.8" }, null, 2)], { type: 'application/json' });
  ...
  const AccountsView = () => {
    ...
    return (
      <div className="space-y-6 w-full animate-fade-in">
        ...
        {/* Total Broker Value Card */}
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-2xl text-white shadow-xl flex justify-between items-center">
          <div>
            <div className="text-blue-100 font-medium mb-2">Valorisation totale (EUR)</div>
            <div className="text-5xl font-bold">{getTotalByBrokerInEur(selectedBroker).toLocaleString('fr-FR', {minimumFractionDigits: 2})} €</div>
          </div>
          <div className="hidden sm:block p-4 bg-white/10 rounded-2xl"><Wallet className="w-12 h-12 text-white" /></div>
        </div>
        
        <div className="flex justify-between items-center mt-8">
          <h3 className="text-xl font-bold text-gray-800 dark:text-white">Comptes</h3>
          <div className="flex items-center gap-3">
            {/* NOUVEAU: Menu de tri */}
            <select
              value={sortConfig.key + (sortConfig.direction === 'asc' ? '-asc' : '-desc')}
              onChange={(e) => {
                const [key, direction = 'desc'] = e.target.value.split('-');
                setSortConfig({ key, direction });
              }}
              className="px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-700 dark:text-gray-300 text-sm shadow-sm"
            >
              {sortOptions.map(opt => (
                <option 
                  key={opt.key + (opt.direction === 'asc' ? '-asc' : '-desc')} 
                  value={opt.key + (opt.direction === 'asc' ? '-asc' : '-desc')}
                >
                  Trier par : {opt.label}
                </option>
              ))}
            </select>
            <button onClick={() => openModal('account')} className="flex items-center gap-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 px-4 py-2 rounded-lg shadow-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700"><PlusCircle className="w-5 h-5 text-blue-600" /> Nouveau compte</button>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {sortedAccounts.map(acc => { 
            ...
            return (
              <div key={acc.id} onClick={() => { setSelectedAccount(acc); setView('snapshots'); }} className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-100 dark:border-slate-700 shadow-lg hover:shadow-xl cursor-pointer flex justify-between items-center group transition-all">
                <div className="flex items-center gap-5">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-gray-50 dark:bg-slate-700 border dark:border-slate-600" style={{color: type?.color}}><Wallet className="w-7 h-7" /></div>
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className="font-bold text-lg text-gray-900 dark:text-white">{acc.name}</h4>
                    ...
                    <div className="flex items-baseline gap-3">
                      <span className="text-xl font-bold text-gray-800 dark:text-gray-200">{getAccountCurrentValueRaw(acc).toLocaleString('fr-FR', {minimumFractionDigits: 2})} {symbol}</span>
                      <PerformanceBadge current={getAccountCurrentValueRaw(acc)} invested={investedTotal} />
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={e => { e.stopPropagation(); openModal('account', acc); }} className="p-2 hover:bg-gray-100 dark:hover:bg-slate-600 rounded-lg text-gray-400 hover:text-blue-600 dark:hover:text-blue-400"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={e => { e.stopPropagation(); deleteAccount(selectedBroker.id, acc.id); }} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg text-gray-400 hover:text-red-500 dark:hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
    ...
    <div className={`min-h-screen font-sans pb-20 w-full transition-colors duration-300 ${darkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-gray-900'}`}>
      <header className="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 sticky top-0 z-30 shadow-md w-full">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1 bg-gray-100 dark:bg-slate-700 p-1 rounded-xl overflow-x-auto">{['dashboard', 'brokers'].map(k => (<button key={k} onClick={() => { setView(k); setSelectedBroker(null); setSelectedAccount(null); }} className={`px-3 sm:px-4 py-1.5 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${view === k ? 'bg-white dark:bg-slate-600 text-blue-600 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}>{k === 'dashboard' ? 'Dash' : k === 'brokers' ? 'Courtiers' : k}</button>))}</nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">{dataLoading ? <div className="flex justify-center py-20"><Loader2 className="w-10 h-10 text-gray-300 animate-spin" /></div> : <>{view === 'dashboard' && <Dashboard />}{view === 'brokers' && <BrokersView />}{view === 'accounts' && <AccountsView />}{view === 'snapshots' && <SnapshotsView />}</>}</main>
