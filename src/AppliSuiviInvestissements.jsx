...
const MovementForm = ({ onSubmit, onCancel, currencySymbol }) => {
    const [data, setData] = useState({ date: new Date().toISOString().split('T')[0], amount: '', type: 'deposit', preValuation: '' });
...
// --- CORRECTIF V1.15 : TRANSFERT PROPORTIONNEL ---
const TransferForm = ({ brokers, onSubmit, onCancel }) => {
  ...
  // Trouver la devise du compte source pour l'affichage
  ...
  return (
      <div className="space-y-4">
          ...
          {/* AJOUT V1.15 : Demande de valorisation si un compte source est sélectionné */}
          {data.sourceId && (
...
const InvestmentTrackerApp = () => {
  ...
  const handleSaveMovement = (data) => {
      ...
      const newMove = { ...data, capitalPart, id: Date.now() };
  ...
  // --- CORRECTIF V1.15 : SAUVEGARDE TRANSFERT INTELLIGENT ---
  const handleSaveTransfer = (data) => {
    // 1. Calculer la part de capital pour le retrait (compte source)
    let withdrawalCapitalPart = parseFloat(data.amount); // Par défaut (fallback)
    
    // Retrouver le compte source pour avoir ses infos
    ...
    if (sourceAccount) {
        ...
        // Règle de 3 pour sortir le bon capital du compte source
        if (valueBefore > 0 && currentInvested > 0) {
            const ratio = withdrawAmount / valueBefore;
    ...
    const withdrawal = { 
        ...
        amount: data.amount, 
        type: 'withdrawal', 
        capitalPart: withdrawalCapitalPart 
    };
  ...
  const handleExport = () => {
    const blob = new Blob([JSON.stringify({ brokers, patrimonyGoal, targetAllocation, version: "1.15" }, null, 2)], { type: 'application/json' });
      ...
      <div className="grid gap-6">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-100 dark:border-slate-700 shadow-md">
           <h3 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white mb-3">
            <Activity className="w-5 h-5 text-indigo-500" /> Qu'est-ce que le TRI ? (Nouveau v1.14)
          </h3>
          ...
          <ul className="list-disc pl-5 space-y-1 text-gray-600 dark:text-gray-300 text-sm">
             ...
             <li>C'est l'indicateur le plus "juste" pour mesurer votre succès d'investisseur.</li>
          </ul>
        </div>

        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-100 dark:border-slate-700 shadow-md">
          <h3 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white mb-3">
            <ArrowRightLeft className="w-5 h-5 text-purple-500" /> Transferts Intelligents (Nouveau v1.15)
          </h3>
          <p className="text-gray-600 dark:text-gray-300 mb-2">
            Lorsque vous transférez de l'argent d'un compte A vers B :
          </p>
          <ul className="list-disc pl-5 space-y-1 text-gray-600 dark:text-gray-300">
             <li>Renseignez la <strong>valeur totale du compte source</strong> dans la fenêtre de transfert.</li>
             <li>L'application calculera automatiquement quelle part de gains est transférée, pour ne pas pénaliser la performance de votre compte source.</li>
          </ul>
        </div>
      </div>
    </div>
